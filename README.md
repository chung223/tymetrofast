...
- ✈ 航班頁籤：桃機出發／抵達看板（本站每 30 分鐘自動更新 TDX FIDS）查航班 → 顯示**航廈／報到櫃台／登機門／行李轉盤** → 一鍵規劃；班機延誤 ≥15 分自動提示並可一鍵依延誤時間重算
- 🚈 路線圖列車位置模擬：依時刻表內插每班車現在位置，手機用**兩排蛇形版面**（免橫向捲動）
- ⚠ 末班車警示（結果卡徽章＋看板末班標記）、💳 TPASS 定期票回本試算（[`data/passes.json`](data/passes.json) 可編輯）
- 📤 行程分享卡：一鍵產生車票風 PNG 分享或下載
- 📍 裝置定位一鍵選離自己最近的車站
- 🕐 依現在時間（台灣時區）或指定日期時間查詢，自動判別平日／假日／國定假日班表
- 📱 行動裝置優先的 RWD、捷運月台看板風格介面；深／淺色模式
- 🔁 時刻表資料化：桃捷改點時改資料檔即可，push 後自動重新部署；每週自動比對官方班表，**改點時自動開 Issue** 摘要差異
- ✅ 部署前煙霧測試：Playwright 實際渲染主要功能通過才上線
